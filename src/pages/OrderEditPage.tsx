...
export default function OrderEditPage() {
  ...
  // Update form data when order loads
  useEffect(() => {
    if (order) {
      setFormData({
  ...
  const updateOrderMutation = useMutation({
    ...
    onError: (error: any) => {
      toast({
        variant: "destructive",
        title: "Lỗi",
        description: error.response?.data?.message || 'Có lỗi xảy ra khi cập nhật đơn hàng',
      })
    }
  })
        ...
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">
              Chỉnh sửa đơn hàng #{order._id.slice(-8).toUpperCase()}
            </h1>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    {formData.items.map((item, index) => {
                      // Use populated data from API if available, otherwise fallback to local data
                      const itemName = item.item?.name || 
                        (item.type === 'product' 
                          ? products.find((p: any) => p._id === item.item)?.name
                          : services.find((s: any) => s._id === item.item)?.name) || 'N/A'
